...
### Added
- **Setup Wizard** — 8-step optional first-run wizard accessible at `/setup` in the web UI. Steps cover: Welcome, Pair V1, Configure WiFi, Configure OBD, Select Driving Mode, Brightness Calibration, Speaker Test, and Backup Configuration. Each step includes a description and actionable detail pointing to the relevant settings page. Wizard state (`done`, `step`) is NVS-persisted and restored on reconnect. Fully skippable via "Skip All" button; navigable via step dots or Back/Next. On completion, a finish screen is shown. A first-run banner appears on the web dashboard until the wizard is completed or dismissed. Wizard can be re-run at any time from the nav or Settings page.
- **Phone Companion API** — external companion apps (Tasker, Automate, etc.) can push real-time telemetry to the device over WiFi via `POST /api/drive/update` (JSON body with optional `speed`/`speed_unit`, `heading`, `gps_accuracy`, `road_name`, `phone_battery`, and required `source`). Data is stored in a new `PhoneCompanionModule` with a 5-second staleness window. When OBD is unavailable, phone speed integrates into `SpeedSourceSelector` as a fallback (priority: OBD=3 → Phone=2 → None=0). `GET /api/drive/status` returns the current snapshot. When valid phone data is present, the dashboard PHONE dot lights green and road name, heading (cardinal + degrees), and phone battery are rendered in the alert-row right side. All fields are optional; only `source` is required.
- **Voice Alert Enhancements** — reduces unnecessary audio chatter with three new filters: *first-alert-only* (suppress repeats of an already-announced encounter), *direction-changes-only* (suppresses bogey-count-change repeats; re-announces only when direction changes), and *per-band voice suppress* (bitmask covering X/K/Ka/Laser — any combination). Also adds startup and shutdown chimes (ascending/descending two-note synthesized tones, each ~110ms, toggle-able independently). Night/quiet volume is already handled by the per-mode `voiceVolume` in Phase 3. Band filter and behaviour flags are NVS-persisted. All controls exposed on the Audio page under "Voice Alert Filters" and "Power Sounds" sections. No existing voice packs, audio system, or custom clips are affected.
- **Smart Brightness Engine** — automatically adjusts display brightness based on alert state, mute state, and idle timeout. Alert boost raises brightness when alerts are active; mute level dims slightly when alerts are muted; idle dim drops to a low level after a configurable timeout (0 = disabled) and wakes immediately on touch, button press, or incoming alert. All five brightness levels (day, night, idle, alert, muted) and the idle timeout are configurable and NVS-persisted. Engine is process-guarded with `#ifndef UNIT_TEST` so native tests are unaffected. New route `GET/POST /api/display/brightness`. New Brightness page in the web UI.
- **Driving Safety Lockout** — blocks dangerous configuration changes (WiFi setup, profile editing, voice settings, display settings, configuration restore) when OBD speed is confirmed above a configurable threshold (default 5 mph). Speed unavailability is treated as "not locked" (safe fallback). Blocked requests receive HTTP 423 with body `{"error":"Unavailable while vehicle is moving"}`. Mute, profile switch, driving mode switch, and brightness remain available at all speeds. Lockout can be disabled and the threshold adjusted via `GET/POST /api/drive/lockout` and the Safety Lockout section on the Driving Modes web page.
- **Persistent Encounter History** — every new alert onset (idle → live transition) is recorded to `/history/encounters.ndjson` on LittleFS and survives reboots. Stores timestamp, band, frequency, direction, bogey count, signal bars, vehicle speed, profile slot, V1 mode, and mute state. Ring buffer trims to 50 entries by default (max 250). Web UI at `/history`: view all encounters in a table, export CSV, clear, and mark individual entries as false alerts. REST API: `GET /api/history`, `POST /api/history/clear`, `GET /api/history/export.csv`, `POST /api/history/mark-false`.
